...
import { 
  ...
  TrendingUp, TrendingDown, BarChart3, Landmark, Receipt
} from 'lucide-react';
...
// Stat Card
const StatCard = ({ title, value, subtitle = '', icon: Icon, trend = 'neutral', trendValue = '' }) => (
...
// ============================================================
const JournalEntriesPage = ({ onShowForm }) => {
  const entries = [
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onShowForm} className="flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">
...
const JournalEntryForm = ({ onClose }) => {
  ...
  const addLine = () => setLines([...lines, { id: lines.length + 1, account: '', debit: 0, credit: 0 }]);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button disabled={!isBalanced} className={`flex items-center gap-2 px-4 py-2 text-sm rounded-lg ${isBalanced ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
            ...
            {lines.map((line, idx) => (
              <tr key={line.id}>
                ...
                <td className="px-4 py-2">
                  <select defaultValue={line.account} className="w-full px-2 py-1.5 text-sm border border-slate-200 rounded">
                ...
                <td className="px-4 py-2">
                  <input type="number" defaultValue={line.debit || ''} placeholder="0.00" className="w-full px-2 py-1.5 text-sm text-right border border-slate-200 rounded" />
                </td>
                <td className="px-4 py-2">
                  <input type="number" defaultValue={line.credit || ''} placeholder="0.00" className="w-full px-2 py-1.5 text-sm text-right border border-slate-200 rounded" />
                </td>
                <td className="px-4 py-2">
                ...
                <td className="px-4 py-2">
                  <button className="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-red-500"><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <table className="w-full">
          <thead className="bg-slate-50 text-xs text-slate-500 uppercase">
            <tr>
              <th className="px-4 py-3 text-center w-12"><input type="checkbox" className="rounded" /></th>
              <th className="px-4 py-3 text-left font-medium">Date</th>
              <th className="px-4 py-3 text-left font-medium">Reference</th>
              <th className="px-4 py-3 text-left font-medium">Description</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {transactions.map((txn) => (
              <tr key={txn.id} className={`hover:bg-slate-50 transition-colors ${txn.matched ? 'bg-emerald-50/30' : ''}`}>
                <td className="px-4 py-3 text-center"><input type="checkbox" className="rounded" checked={txn.matched} readOnly /></td>
                ...
                <td className="px-4 py-3 text-center">
                  {txn.matched ? (
                    <span className="inline-flex items-center gap-1 text-emerald-600 text-xs"><CheckCircle size={14} /> Matched</span>
                  ) : (
...
const FiscalPeriodsPage = () => {
  const periods = [
    { period: 1, name: 'Jan', status: 'closed' },
    { period: 2, name: 'Feb', status: 'closed' },
    { period: 3, name: 'Mar', status: 'closed' },
    { period: 4, name: 'Apr', status: 'closed' },
    { period: 5, name: 'May', status: 'closed' },
    { period: 6, name: 'Jun', status: 'closed' },
    { period: 7, name: 'Jul', status: 'closed' },
    { period: 8, name: 'Aug', status: 'closed' },
    { period: 9, name: 'Sep', status: 'closed' },
    { period: 10, name: 'Oct', status: 'closed' },
    { period: 11, name: 'Nov', status: 'open' },
    { period: 12, name: 'Dec', status: 'open' },
  ];
          ...
          {periods.map(p => (
            <div 
              ...
              className={`p-3 rounded-lg border text-center ${
                p.status === 'open' ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'
              }`}
            >
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200">
        ...
        <div className="px-6">
          <Tabs tabs={tabs} active={activeTab} onChange={(id) => { setActiveTab(id); setShowJournalForm(false); }} />
        </div>
      </div>

      <div className="p-6">
        {activeTab === 'coa' && <ChartOfAccountsPage />}
        {activeTab === 'journals' && !showJournalForm && <JournalEntriesPage onShowForm={() => setShowJournalForm(true)} />}
        {activeTab === 'journals' && showJournalForm && <JournalEntryForm onClose={() => setShowJournalForm(false)} />}
